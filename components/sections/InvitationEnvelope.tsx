...
export default function InvitationEnvelope({
  ...
  const sealed = !revealed ? (
    <motion.button
      ...
      variants={{
        closed: { opacity: 1 },
        open: { opacity: 0, transition: { delay: 1.55, duration: 0.55, ease: EASE } },
      }}
      className={cn(
        "z-10 flex cursor-pointer flex-col items-center justify-center px-6 outline-none focus-visible:ring-2 focus-visible:ring-charcoal/40 focus-visible:ring-offset-2 focus-visible:ring-offset-cream",
        isHero
          ? "fixed inset-0 z-[60] bg-cream py-10 sm:py-12"
    ...
    >
      ...
      <motion.span
        variants={{
          closed: { opacity: 1, y: 0 },
          open: { opacity: 0, y: 18, transition: { delay: 1.25, duration: 0.5, ease: EASE } },
        }}
        style={{ perspective: 1500 }}
        className="relative block aspect-[7/5] w-full [transform-style:preserve-3d]"
      >
        <motion.span
          aria-hidden
          animate={open ? undefined : { y: [0, -5, 0] }}
          transition={open ? undefined : { duration: 3.8, repeat: Infinity, ease: "easeInOut" }}
        ...
        <motion.span
          ...
          variants={{
            closed: { rotateX: 0 },
            open: { rotateX: -175, transition: { delay: 0.06, duration: 0.62, ease: EASE } },
          }}
          style={{
            zIndex: 40,
        ...
        <motion.span
          ...
          variants={{
            ...
            open: { scale: 1.35, opacity: 0, y: -10, transition: { duration: 0.34, ease: EASE } },
          }}
      ...
      <motion.span
        variants={{
          closed: { opacity: 1, y: 0 },
          open: { opacity: 0, y: 10, transition: { duration: 0.28, ease: EASE } },
        }}
        animate={open ? undefined : { opacity: [0.62, 1, 0.62] }}
        transition={open ? undefined : { duration: 2.4, repeat: Infinity, ease: "easeInOut" }}
        className={cn(
          "mt-1 rounded-full border border-charcoal/25 bg-cream/80 px-6 py-2.5 font-medium uppercase tracking-[0.24em] text-charcoal/75 backdrop-blur-[2px]",
  ...
  return (
    <InvitationRevealProvider revealed={revealed}>
      <div className={cn("relative min-w-0 max-w-full", className)}>
        <motion.div
          initial={false}
          animate={open ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: 36, scale: 0.98 }}
